...
ScoreAI::ScoreAI(std::string const & name,size_t maxTiles):
...
void ScoreAI::init(Dictionary* dict) { 
...
Move* ScoreAI::getMove(const Board & board, Player& player){
		...
		if (moves.empty()) { 
			...
			std::vector<std::string> bestMove = moves[maxScoreIndex]; 
...
void ScoreAI::horizontalHelper(int row, int col, int init_row, int init_col, 
...
void ScoreAI::verticalHelper(int row, int col, int init_row, int init_col, 
							   std::string used, std::string tmp, std::string toUse, 
							   Player& player, const Board& board) 
{ 
